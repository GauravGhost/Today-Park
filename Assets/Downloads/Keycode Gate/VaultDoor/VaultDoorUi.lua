...
local myDoorController = nil

function SetVisible(state, teleporter)
    NumpadElement:EnableInClassList("hide", not state)
end

function self:Awake()
    myDoorController = self.gameObject:GetComponent(VaultDoorScript)
    SetVisible(false)
end
...
function checkKey()
    if currentNumber == PassKey then 
        ...
        Timer.After(1, function() SetVisible(false); myDoorController.SendGateRequest() end)
        Entered = true 
    else 
        inputLabel:SetPrelocalizedText("DECLINED")
        Audio:PlayShader(decAudio)
        Timer.After(1, function() SetVisible(false) end)
        Entered = true 
    end
end
